    ...
    do {
        ...
        switch (pilihan) {
            case 1: {
            	...
            	int numBooks = 0;
    			address P = First(bukuList);
    			while (P != NULL) {
        			numBooks++;
        			P = Next(P);
			    }
    			OrangList = ReallocOrangList(OrangList, numBooks - 1); // 0-based index
			...
			case 2: {
			    ...
			    int bookIndex = countBook(bukuList, title) - 1;
			    if (bookIndex < 0) {
			        printf("Book index calculation error.\n");
			        waitForEnter();
			        break;
			    }
			    
			    // Make sure OrangList is allocated for this book
			    OrangList = ReallocOrangList(OrangList, bookIndex);
			    
			    if (bookIndex >= orangListSize) {
			        printf("Book queue not initialized correctly.\n");
			        waitForEnter();
			        break;
			    }
			    ...
			    if (!IsEmpty(*OrangList[bookIndex])) {
			        printf("\nProcessing queue for book '%s'...\n", title);
			        processQueue(OrangList[bookIndex], AddStock, NewStock);
			    }
			...
			case 3: {
                ...
                address TheBook = Search(bukuList, title);
                if (TheBook == NULL) {
     			   	printf("Book not found!\n");
        			waitForEnter();
			        break;
    			...
    			int bookIndex = countBook(bukuList, title) - 1;
                if (bookIndex < 0 || bookIndex >= orangListSize) {
        			printf("Error locating book.\n");
        			waitForEnter();
        			break;
			    }
				...
				if (Int(TheBook) > 0 && IsEmpty(*OrangList[bookIndex])) {
        			...
        			InsertPriority(OrangList[bookIndex], borrower);
            ...
            case 4: {
				printOrangList(OrangList, bukuList);
                waitForEnter();
                break;
            }
            case 5: {
			    ...
			    address returnedBook = Search(bukuList, judul);
			    if (returnedBook != NULL) {
			        Int(returnedBook)++; // Increment stock by 1
			        int bookIndex = countBook(bukuList, judul) - 1;
			        ...
			        if (bookIndex >= 0) {
			            ...
			            // Check if there are people in queue who can now borrow the book
			            if (bookIndex < orangListSize && !IsEmpty(*OrangList[bookIndex])) {
			                printf("\nProcessing queue for book '%s'...\n", judul);
			                processQueue(OrangList[bookIndex], returnedBook, 1); // Process for the 1 returned book
			            } else {
			...
			case 7: {
				...
				bookIndex = countBook(bukuList, bookTitle) - 1;
				if (bookIndex < 0 || bookIndex >= orangListSize) {
					printf("Book not found or queue not initialized!\n");
					waitForEnter();
					break;
				}
				...
				address prev = NULL;
				address current = First(*OrangList[bookIndex]);
				boolean found = false;
				int priority = 0;
				...
				while (current != NULL && !found) {
					if (strcmp(Info(current), name) == 0) {
						...
						if (prev == NULL) {
							...
							DeleteFirst(OrangList[bookIndex], &current);
						} else {
							// In middle/end of queue
							DeleteAfter(OrangList[bookIndex], &current, prev);
						}
						Dealokasi(current);
